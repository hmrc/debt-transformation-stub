  ...
  ".filterAndEncodeList()" - {
    "given a list of LockTypeAndLock containing all 4 lock types should correctly encode them in a appropriate application.conf file format" in {
      val locks = List(
        ...
        LockTypeAndLock(
          "payments",
          EtmpLock("audit/compliance", disallowPaye = false, disallowVat = true, disallowSa = true, disallowSimp = true)
        )
      )
    ...
    "given a list of LockTypeAndLock containing some lock types and some unknown lockTypes, should correctly encode them in a appropriate application.conf file format, leaving empty known and unknown types empty" in {
      val locks = List(
        LockTypeAndLock(
          "dunning ",
          EtmpLock(
