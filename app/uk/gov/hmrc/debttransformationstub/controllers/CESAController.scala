...
class CESAController @Inject() (environment: Environment, cc: ControllerComponents)
  ...
  def PaymentPlanEligibilityDmRequest(): Action[JsValue] = Action.async(parse.json) { implicit rawRequest: Request[JsValue] =>
    withCustomJsonBody[PaymentPlanEligibilityDmRequest] { request =>
      val fileName = s"$basePath.eligibilityDm/${request.chargeReferences}.json"
      environment.getExistingFile(fileName) match {
        case _ if request.chargeReferences.equals("cesaChargeSource") =>
