...
import uk.gov.hmrc.debttransformationstub.models.CdcsChargeInfoRequest
...
class CDCSChargeInfoController @Inject() (environment: Environment, cc: ControllerComponents)
    extends BackendController(cc) with CustomBaseController {

  private lazy val logger = new RequestAwareLogger(this.getClass)
  private val basePath = "conf/resources/data/cdcsChargeInfo"

  def cdcsChargeInfo(): Action[JsValue] = Action.async(parse.json) { implicit rawRequest: Request[JsValue] =>
    withCustomJsonBody[CdcsChargeInfoRequest] { request =>
